...
abstract class Convert {
  ...
  protected function convert_texte_pub() {
    ...
    // dispatch left / right
    if ($position == 'left' ) {
      $left_meta   = '{"width":33.33}';
      $left_styles = 'style="flex-basis:33.33%"';
      $left_markup = $ad_markup;

      $right_markup = $content_markup;
      $right_meta   = '{"width":66.66}';
      $right_styles = 'style="flex-basis:66.66%"';
    } else {
      $left_meta   = '{"width":66.66}';
      $left_styles = 'style="flex-basis:66.66%"';
      $left_markup = $content_markup;

      $right_markup = $ad_markup;
      $right_meta   = '{"width":33.33}';
      $right_styles = 'style="flex-basis:33.33%"';
    }
    ...
    return '
      <!-- wp:columns -->
        <div class="wp-block-columns">
          <!-- wp:column ' . $left_meta . ' -->
            <div class="wp-block-column" ' . $left_styles . '>
              ' . $left_markup . '
            </div>
          <!-- /wp:column -->
          <!-- wp:column ' . $right_meta . ' -->
            <div class="wp-block-column" ' . $right_styles . '>
              ' . $right_markup . '
            </div>
          <!-- /wp:column -->
        </div>
  ...
  protected function convert_espace_pub() {
    return '<!-- wp:xyoos/adaptable /-->';
  }
